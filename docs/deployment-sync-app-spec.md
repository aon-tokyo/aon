...
### レンタルサーバーが「増分（マニフェスト）」に不向きな場合

一部の共有レンタルでは、接続の不安定さや実装の差により **変更検知をスキップしたくない**ことがある。その場合は **`uploadMode: full`**（CLI: `--full-upload`）とし、**マニフェストによる省略を行わず**、マッチした全ファイルを毎回 PUT する（サーバー上の不要ファイルの削除は引き続き既定では行わない）。
...
*文書バージョン: 0.3 — `uploadMode: full` とレンタル向け FTP/SFTP 互換オプションを追記。*
