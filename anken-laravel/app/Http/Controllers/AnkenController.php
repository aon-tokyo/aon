...
class AnkenController extends Controller
{
    ...
    public function search(Request $request)
    {
        ...
        $list = collect(self::ANKEN)->filter(function ($a) use ($q, $role, $style, $langs, $fws, $minRate, $pref, $city, $station, $remoteOk) {
            ...
            if (!$remoteOk && $style !== '' && $a['style'] !== $style) return false;

            // 言語 (OR)
            if (!empty($langs) && !array_intersect($langs, $a['langs'])) return false;

            // フレームワーク (OR)
            if (!empty($fws) && !array_intersect($fws, $a['fws'])) return false;
    ...
    {
        $sc = 0;
        foreach ($langs as $l) { if (in_array($l, $a['langs'], true)) $sc += 3; }
        foreach ($fws   as $f) { if (in_array($f, $a['fws'],   true)) $sc += 2; }
        if ($a['hot'])      $sc += 1;
        if ($a['posted'] <= 1) $sc += 1;
        return $sc;
    }
